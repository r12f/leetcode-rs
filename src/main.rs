...
fn fix_default_code(return_type: &str, code: &str) -> String {
    let mut code = code.to_string();
    code = code.replace("impl Solution", "#[solution]\nimpl Solution");

    let comment_re = Regex::new(r"// [^\n]*\n").unwrap();
    code = comment_re.replace_all(&code, "").to_string();
...
fn deal_problem(problem: &Problem, code: &CodeDefinition, write_mod_file: bool) {
    ...
    let source = template
        ...
        .replace(
            ...
            &fix_default_code(&problem.return_type, &code.default_code),
        )
